...
public class PageVO {
	...
	private void calcPage() {
		// 이 메소드가 모든 변수 값을 계산합니다. 이때 이메서드가 시작될때, page=2, totalCount=96, perpageNum=10은
		// 이미 결정이 된 상태입니다.
		// DB쿼리에서 사용... 시작데이터번호 = (jsp클릭한 페이지번호-1)*페이지당 보여지는 개수
		startNo = (this.page - 1) * perPageNum; // 2페이지

		// page변수는 현재 jsp에서 클릭한 페이지번호
		int tempEnd = (int) (Math.ceil(page / (double) this.perPageNum) * this.perPageNum);
		// page 1 1/10=0.1 = 1*10=10
		// page 2 2/10=0.2 = 1*10= 10
		//
		// page 10 10/10=1.0 1*10=10
		// page 11 11/10=1.1 2*10=20;
		// page 12 12/10=1.2 2*10=20;
